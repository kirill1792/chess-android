...
public class MoveValidator {
    ...
    public List<Coordinates> findFinalMoves(){
        ...
        for (Coordinates move: possibleMoves){
            if(canMove(move, board, selectedFigure)){
                finalMoves.add(move);
            }
        }
    ...
    public boolean canMove(Coordinates coordinatesToMove, Board board, Figure figure) {
        ...
        for (Coordinates coordinates : result) {
            if (coordinates.equals(coordinatesToMove)) {
                ...
                return !checkForCheck(findKing(copyBoard), copyBoard);
            }
        }
        return false;
    }

    private Coordinates findKing(Board copyBoard) {
        for (int i = 0; i < copyBoard.getFields().size(); i++) {
            for (int j = 0; j < copyBoard.getFields().get(i).size(); j++) {
                if(copyBoard.getFields().get(i).get(j) != null && copyBoard.getFields().get(i).get(j) instanceof King && copyBoard.getFields().get(i).get(j).color.equals(selectedFigure.color)){
    ...
    private Rook checkSide(Coordinates cords, King king){
        ...
        for (int i = 0; i < 2; i++){
            ...
            if (checkForCheck(new Coordinates(startKingCords.getRow(), current), copyBoard)){
                return null;
            }
            current += buff;
        }
